...
public class AdminController {
    ...
    public PageResponse<Map<String, Object>> listRestaurants(
        ...
        return adminService.listRestaurants(status, search, pg);
    }

    @GetMapping("/drivers")
    public PageResponse<Map<String, Object>> listDrivers(
            @RequestParam(required = false) UserStatus status,
            @RequestParam(required = false) String search,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        Pageable pg = PageRequest.of(Math.max(page, 0), Math.max(size, 1));
        return adminService.listDrivers(status, search, pg);
    }
    ...
    public PageResponse<Map<String, Object>> listUsers(
            ...
            @RequestParam(defaultValue = "20") int size) {
        Pageable pg = PageRequest.of(Math.max(page, 0), Math.max(size, 1));
        return adminService.listUsers(role, search, pg);
    }
