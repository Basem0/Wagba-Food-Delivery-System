...
function catImage(name) {
  ...
  const lock =
    (name.split("").reduce((a, ch) => a + ch.charCodeAt(0), 0) % 900) + 300;
...
function renderSortMenu() {
  ...
  if (!m) return;
  m.innerHTML = SORT_OPTS.map(
    (o) =>
      `<button type="button" class="dd-item ${o.v === (window._sort || "name") ? "active" : ""}" onclick="pickSort('${o.v}')">${o.l}</button>`,
  ).join("");
}
function toggleDD(name) {
  const dd = document.getElementById(
    "dd" + name.charAt(0).toUpperCase() + name.slice(1),
  );
  if (!dd) return;
...
function pickCuisine(val) {
  ...
  document
    .querySelectorAll("#cuisineMenu .dd-item")
    .forEach((it) =>
      it.classList.toggle(
        "active",
        it.getAttribute("onclick") === `pickCuisine('${val}')`,
      ),
    );
  closeDDs();
  applyFilters();
...
function pickSort(val) {
  ...
  if (lbl)
    lbl.textContent = (SORT_OPTS.find((o) => o.v === val) || {}).l || val;
  document
    .querySelectorAll("#sortMenu .dd-item")
    .forEach((it) =>
      it.classList.toggle(
        "active",
        it.getAttribute("onclick") === `pickSort('${val}')`,
      ),
    );
  closeDDs();
  applyFilters();
...
function init() {
  ...
  const spN = document.getElementById("spNameText"),
    spE = document.getElementById("spEmailText"),
    spA = document.getElementById("spAvatar");
  if (spN) spN.textContent = me.name || me.email || "Customer";
  if (spE) spE.textContent = me.email || "";
  if (spA)
    spA.textContent = (me.name || me.email || "C")
      .trim()
      .charAt(0)
      .toUpperCase();
  window.onNav = (v) => {
...
function renderSettings() {
  ...
  root.innerHTML = renderSettingsShell({
    panel: addressCardHtml(),
    label: "Address",
    icon: "geo-alt",
  });
  loadAccountSettings();
  loadAddress();
  window.__onSettingsCat = (c) => {
    if (c === "role") {
      if (!addrMap) initAddressMap();
      else setTimeout(() => addrMap.invalidateSize(), 60);
    }
  };
}
...
function addrUseMyLocation() {
  if (!navigator.geolocation) {
    toast("Location", "Geolocation not supported", "err");
    return;
  }
...
async function loadAddress() {
  try {
    ...
    set("addrCity", a.city);
    set("addrStreet", a.street);
    set("addrBuilding", a.buildingNumber);
    set("addrApartment", a.apartment);
    set("addrDetails", a.details);
    ...
    /* settings still usable without saved address */
  }
}
...
async function saveAddress() {
  ...
  const body = {
    city: get("addrCity"),
    street: get("addrStreet"),
    buildingNumber: get("addrBuilding"),
    apartment: get("addrApartment"),
    details: get("addrDetails"),
  };
  body.latitude = addrLat;
  body.longitude = addrLng;
  if (!body.city || !body.street) {
    showAlert("addrAlert", "warning", "City and street are required");
    return;
  }
  if (body.latitude == null || body.longitude == null) {
    showAlert("addrAlert", "warning", "Please drop a pin on the map");
    return;
  }
  try {
    ...
    showAlert("addrAlert", "danger", e.message);
  }
}
        ...
        .map((c) => {
          ...
          const ic =
            c.name === "All"
              ...
              : `<img class="cc-img" src="${escapeHtml(catImage(c.name))}" onerror="this.style.display='none'">`;
...
function selectCategory(name) {
  ...
  document
    .querySelectorAll("#catChips .cat-chip")
    .forEach((c) =>
      c.classList.toggle(
        "active",
        c.getAttribute("onclick") === `selectCategory('${name}')`,
      ),
    );
  applyFilters();
}
...
function applyFilters() {
  ...
  if (sort === "rating")
    ...
    list = [...list].sort(
      (a, b) => (a.minOrderTotal || 0) - (b.minOrderTotal || 0),
    );
  else
...
async function viewRestaurant(id) {
  try {
    ...
    document.getElementById("foodRestaurantName").textContent =
      r.name || "Menu";
    ...
    document.getElementById("restStatus").innerHTML = r.status
      ? statusBadge(r.status.name || r.status)
      : "";
    window._foods = foods;
...
function foodCardHtml(f) {
  const pic = f.imageUrl
    ? `<img src="${escapeHtml(f.imageUrl)}" onerror="onImgError(this)" data-label="${escapeHtml(f.name)}">`
...
function renderFoodCats() {
  ...
  const cats = [
    ...new Set(
      (window._foods || []).map((f) => f.categoryName).filter(Boolean),
    ),
  ];
  if (!cats.length) {
    el.innerHTML = "";
  ...
  const tabs = [{ id: "__all__", name: "All" }].concat(
    cats.map((c) => ({ id: c, name: c })),
  );
  el.innerHTML = tabs
    .map(
      (c) =>
        `<button type="button" class="cat-tab ${c.id === window._foodCat ? "active" : ""}" onclick="selectFoodCat('${escapeHtml(c.id)}')">${escapeHtml(c.name)}</button>`,
    )
    .join("");
}
...
function renderFoodItems() {
  ...
  if (!el) return;
  const list = (window._foods || []).filter(
    (f) => window._foodCat === "__all__" || f.categoryName === window._foodCat,
  );
  if (!list.length) {
    el.innerHTML = emptyState(
      "bi-egg-fried",
  ...
  try {
    ...
    if (items.length && restaurantId != null) {
      const other = items.find(
        (it) => it.restaurantId != null && it.restaurantId !== restaurantId,
      );
      if (other) {
        ...
        if (!ok) {
          toast("Cancelled", "Item not added so your cart stays as is");
          return;
        }
...
function openFoodModal(id) {
  ...
  if (!f) return;
  _fmId = id;
  document.getElementById("fmPic").innerHTML = f.imageUrl
    ? `<img src="${escapeHtml(f.imageUrl)}" onerror="onImgError(this)" data-label="${escapeHtml(f.name)}">`
    : `<i class="bi bi-egg-fried"></i>`;
  document.getElementById("fmCat").innerHTML =
    `${f.categoryName ? `<span class="badge-soft">${escapeHtml(f.categoryName)}</span>` : ""}${f.offer ? `<span class="offer-chip sm"><i class="bi bi-tag-fill"></i> Offer</span>` : ""}`;
  document.getElementById("fmName").textContent = f.name || "";
  document.getElementById("fmDesc").textContent =
  ...
  if (f.available) addBtn.classList.remove("d-none");
...
async function loadCart() {
  try {
    ...
    if (!c.items || !c.items.length) {
      ...
      if (ncc) ncc.classList.add("d-none");
      syncBottomBadge("cartCount");
      const ci = document.getElementById("checkoutCard");
      if (ci) ci.classList.add("d-none");
      return;
    }
    if (pill) {
      pill.classList.remove("d-none");
      const pt = document.getElementById("cartTotalPill");
      if (pt) pt.textContent = money(c.total);
    }
    if (cnt) {
    ...
    const ncc = document.getElementById("navCartCount");
    if (ncc) {
      ncc.classList.remove("d-none");
      ncc.textContent = c.items.length;
    }
    syncBottomBadge("cartCount");
    const ci = document.getElementById("checkoutCard");
    if (ci) ci.classList.remove("d-none");
    window.cartTotal = c.total;
    const ct = document.getElementById("cartCheckoutTotal");
    if (ct) ct.textContent = money(c.total);
    initCheckoutMap();
    setTimeout(() => {
...
async function applyCartCoupon() {
  ...
  try {
    ...
    if (msg) {
      ...
      msg.textContent = "Coupon applied · saved " + money(r.discount);
    }
    const dr = document.getElementById("cartDiscountRow");
    if (dr) dr.style.display = "flex";
    const d = document.getElementById("cartDiscount");
    if (d) d.textContent = "-" + money(r.discount);
    const ft = document.getElementById("cartFinalTotal");
    if (ft) ft.textContent = money(r.finalTotal);
    const ct = document.getElementById("cartCheckoutTotal");
    if (ct) ct.textContent = money(r.finalTotal);
  } catch (e) {
    ...
    const ft = document.getElementById("cartFinalTotal");
    if (ft) ft.textContent = money(window.cartTotal || 0);
    const ct = document.getElementById("cartCheckoutTotal");
    if (ct) ct.textContent = money(window.cartTotal || 0);
  }
}
async function loadMyCouponsForCart() {
  ...
  try {
    ...
    box.innerHTML =
      `<span class="small muted me-1">Your coupons:</span>` +
      usable
        .map(
          (c) => `
      <button type="button" class="coupon-chip" onclick="useMyCoupon('${escapeHtml(c.code)}')">${escapeHtml(c.code)} · ${c.discountType === "PERCENTAGE" ? c.value + "%" : money(c.value)}</button>`,
        )
        .join("");
  } catch (e) {
    box.innerHTML = "";
...
function useMyCoupon(code) {
  const inp = document.getElementById("cartCoupon");
...
async function updateQty(itemId, foodId, qty) {
  ...
  try {
    await api("/cart/items/" + itemId, "PUT", {
      foodId,
...
function selectPay(val) {
  const inp = document.getElementById("ckPayment");
...
async function prefillCheckoutAddress() {
  try {
    ...
    set("ckCity", a.city);
    set("ckStreet", a.street);
    set("ckBuilding", a.buildingNumber);
    set("ckApartment", a.apartment);
    set("ckDetails", a.details);
    if (a.latitude != null && a.longitude != null) {
      window._ckLat = a.latitude;
      window._ckLng = a.longitude;
...
async function submitCheckout() {
  ...
  const body = {
    ...
    couponCode:
      (document.getElementById("cartCoupon")?.value || "").trim() ||
      appliedCouponCode ||
      null,
  ...
  try {
    ...
    showAlert("checkoutAlert", "danger", e.message);
  }
}
...
async function startVisaPayment(body) {
  ...
  try {
    ...
    _payDev = cfg.devMode;
    document.getElementById("payDev").style.display = _payDev
      ? "block"
      : "none";
    document.getElementById("stripeCard").style.display = _payDev
      ? "none"
      : "block";
    if (!_payDev) {
...
async function confirmPayment() {
  ...
  try {
    const { error, paymentIntent } = await _stripe.confirmCardPayment(
      _clientSecret,
      { payment_method: { card: _card } },
    );
    if (error) {
      showAlert("payAlert", "danger", error.message);
      return;
    }
    if (
      paymentIntent &&
    ...
    showAlert("payAlert", "danger", e.message);
  }
}
async function finishVisaOrder(paymentReference) {
  ...
  try {
    ...
    const order = await api("/orders/checkout", "POST", body);
    if (paymentReference) {
      await api("/payments/confirm", "POST", {
        orderId: order.id,
        paymentReference,
      });
    }
...
function setStar(id, v) {
  ...
  document
    .querySelectorAll("#stars-" + id + " .star")
    .forEach((st) => st.classList.toggle("on", parseInt(st.dataset.v) <= v));
}
...
async function openOrderDetail(id) {
  ...
  let reviews = [];
  try {
    reviews = await api("/reviews/order/" + id);
  } catch (e) {
    reviews = [];
  }
...
function renderOrderDetail(o, reviews) {
  ...
  let itemsHtml = o.items
    .map((it) => {
      const itemRv = reviews.find(
        (r) => r.type === "ITEM" && r.targetId === it.foodId,
      );
...
async function submitDetailReview(
  ...
  if (rating < 1) {
    toast("Rate first", "Please choose a star rating", "danger");
    return;
  }
  try {
...
function renderCouponTabs() {
  ...
  const valid = _couponList.filter(
    (c) => !c.used && (!c.expiryDate || new Date(c.expiryDate) >= now),
  ).length;
  const used = _couponList.filter((c) => c.used).length;
  const expired = _couponList.filter(
    (c) => !c.used && c.expiryDate && new Date(c.expiryDate) < now,
  ).length;
  el.innerHTML = `
...
async function loadFavorites() {
  try {
    ...
    el.innerHTML = `${header}<div class="favorites-grid">${list
      .map((f) => {
        const rating =
          f.avgRating != null ? Number(f.avgRating).toFixed(1) : null;
        return `
...
async function toggleFavorite(restaurantId) {
  try {
    ...
    if (res.favorited) window._favIds.add(restaurantId);
    else window._favIds.delete(restaurantId);
    ...
    if (
      ...
      !document.getElementById("view-favorites").classList.contains("d-none")
    )
      loadFavorites();
  } catch (e) {
    showAlert("alertBox", "danger", e.message);
...
async function loadTracking(id) {
  try {
    ...
    const live = d != null && d.latitude != null && d.longitude != null;
    ...
    driverEl.innerHTML = d
        ...
        <div class="driver-info">
          ...
          <div>
            ...
            <div class="muted small">
              ${escapeHtml(d.vehicleType || "")}
              ·
              ${escapeHtml(d.vehicleNumber || "")}

              ${
                live && d.locationUpdatedAt
                  ? `
                    ·
                    <i class="bi bi-broadcast"></i>
                    ${fmtTimeAgo(d.locationUpdatedAt)}
                  `
                  : ""
              }
            </div>
      ...
      : `
        <div class="alert alert-info mb-0">
          ...
          Driver not assigned yet — tracking will appear once a driver
          accepts your order.
        </div>
      `;
    ...
    const custLat = t.customerLatitude;
    const custLng = t.customerLongitude;

    const restLat = t.restaurantLatitude;
    const restLng = t.restaurantLongitude;

    const points = [];

    // Add driver location only when it actually exists
    if (live) {
      ...
      if (mapEl) {
        mapEl.innerHTML = `
          <div class="map-empty">
            ...
            <div>
              Live map appears here once the driver shares
              their location.
            </div>
          </div>
        `;
    ...
    if (live) {
      if (!trackMarker) {
        trackMarker = L.marker([d.latitude, d.longitude], {
          icon: L.divIcon({
        ...
        trackMarker.setLatLng([d.latitude, d.longitude]);
      }
    } else {
      // Driver exists but has no live location
    ...
    if (custLat != null && custLng != null) {
      if (!trackCustMarker) {
        trackCustMarker = L.marker([custLat, custLng], {
          icon: L.divIcon({
            className: "pin-cust",
        ...
        trackCustMarker.setLatLng([custLat, custLng]);
      }
    }
    ...
    if (restLat != null && restLng != null) {
      if (!trackRestMarker) {
        trackRestMarker = L.marker([restLat, restLng], {
          icon: L.divIcon({
            className: "pin-rest",
          ...
          .addTo(trackMap)
          .bindPopup(
            escapeHtml(t.restaurantName || "Restaurant") + " (pickup)",
          );
      } else {
        trackRestMarker.setLatLng([restLat, restLng]);
      }
    }
    ...
    // -------------------------------------------------
    if (points.length > 1) {
      trackMap.fitBounds(points, {
        padding: [40, 40],
        maxZoom: 15,
      });
    } else {
      trackMap.setView(points[0], 15);
    }

    // Leaflet needs this when the map is inside a modal
    setTimeout(() => {
      if (trackMap) {
    ...
    showAlert("trackAlert", "danger", e.message);
  }
}
function buildTimeline(orderStatus, deliveryStatus) {
  const steps = [
    ...
    {
      label: "Preparing",
      done: ["PREPARING", "READY", "OUT_FOR_DELIVERY", "DELIVERED"].includes(
        orderStatus,
      ),
      icon: "egg-fried",
    },
  ...
  return (
    ...
    steps
      .map(
        (s) => `
    <div class="step ${s.done ? "done" : ""}"><div class="dot"><i class="bi bi-${s.icon}"></i></div><div class="lbl">${s.label}</div></div>`,
      )
      .join("") +
    "</div>"
  );
